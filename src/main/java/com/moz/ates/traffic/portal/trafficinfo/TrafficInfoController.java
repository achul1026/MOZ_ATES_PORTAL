...
public class TrafficInfoController {
	...
	public String trafficLawInfoDetail(Model model, MozTfcLwInfo tfcLwInfo) throws Exception {
		
		if (MozatesCommonUtils.isNull(tfcLwInfo.getTfcLawId())) {
			throw new Exception();
		}
		MozTfcLwInfo tfcLwInfoDetail = trafficLawInfoService.getTrafficLawDetail(tfcLwInfo);
		
		if (MozatesCommonUtils.isNull(tfcLwInfoDetail)) {
			throw new Exception();
		}
		
		model.addAttribute("tfcLwInfoDetail", tfcLwInfoDetail);
		model.addAttribute("tfcLwFineInfoList", trafficLawInfoService.getLawFineInfoList(tfcLwInfo));
